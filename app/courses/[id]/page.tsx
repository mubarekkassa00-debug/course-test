...
// Header background image (Quran / Islamic manuscript)
...
export default function CoursePage() {
  ...
  // Timeline: lessons + final exam only (weekly quizzes removed per request)
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white transition-colors duration-300 font-sans">
      ...
      <header className="sticky top-0 z-40 shadow-md">
        ...
        <div className="absolute inset-0 overflow-hidden">
          ...
          />
          <div className={`absolute inset-0 bg-gradient-to-r ${course.gradient} opacity-80`} />
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-5">
          ...
          <div className="flex flex-wrap gap-2">
            <span className="bg-white/20 backdrop-blur text-white text-xs px-3 py-1 rounded-full border border-white/20">
              {course.category}
            </span>
            <span className="bg-yellow-400/90 text-yellow-900 text-xs px-3 py-1 rounded-full flex items-center gap-1 font-medium">
              <Award className="h-3 w-3" /> {amh.certificateBadge}
            </span>
            <span className="bg-white/20 backdrop-blur text-white text-xs px-3 py-1 rounded-full border border-white/20 flex items-center gap-1">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-slate-900 dark:text-white truncate">
                  {item.type === 'lesson' ? item.data.title : amh.finalExam}
                </p>
              </div>
              {item.type === 'lesson' ? (
                <Link
                  href={`/courses/${courseId}/lessons/${item.data.id}`}
                  className="flex-shrink-0 px-4 py-2 rounded-xl bg-emerald-600 text-white text-sm font-medium hover:bg-emerald-700 transition flex items-center gap-1"
                >
                  <PlayCircle className="h-4 w-4" />
                  <span className="hidden sm:inline">{amh.startLesson}</span>
                </Link>
              ) : (
                <button
                  type="button"
